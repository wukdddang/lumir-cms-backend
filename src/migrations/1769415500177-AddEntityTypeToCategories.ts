...
export class AddEntityTypeToCategories1769415500177 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='wiki_file_systems' AND column_name='type') 
                THEN
                    ALTER TABLE "wiki_file_systems" ADD "type" "public"."wiki_file_systems_type_enum" NOT NULL;
                    COMMENT ON COLUMN "wiki_file_systems"."type" IS '타입 (folder|file)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='wiki_permission_logs' AND column_name='action') 
                THEN
                    ALTER TABLE "wiki_permission_logs" ADD "action" "public"."wiki_permission_logs_action_enum" NOT NULL;
                    COMMENT ON COLUMN "wiki_permission_logs"."action" IS '처리 상태 (detected|removed|notified|resolved)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                ...
                THEN
                    ALTER TABLE "categories" ADD "entityType" "public"."categories_entitytype_enum" NOT NULL;
                    COMMENT ON COLUMN "categories"."entityType" IS '엔티티 타입 (announcement|main_popup|shareholders_meeting|...)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='survey_questions' AND column_name='type') 
                THEN
                    ALTER TABLE "survey_questions" ADD "type" "public"."survey_questions_type_enum" NOT NULL;
                    COMMENT ON COLUMN "survey_questions"."type" IS '질문 타입 (short_answer|paragraph|multiple_choice|...)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='attendees' AND column_name='status') 
                THEN
                    ALTER TABLE "attendees" ADD "status" "public"."attendees_status_enum" NOT NULL DEFAULT 'pending';
                    COMMENT ON COLUMN "attendees"."status" IS '수강 상태 (pending|in_progress|completed|overdue)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='education_managements' AND column_name='status') 
                THEN
                    ALTER TABLE "education_managements" ADD "status" "public"."education_managements_status_enum" NOT NULL DEFAULT 'scheduled';
                    COMMENT ON COLUMN "education_managements"."status" IS '상태 (scheduled|in_progress|completed|cancelled|postponed)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='vote_results' AND column_name='result') 
                THEN
                    ALTER TABLE "vote_results" ADD "result" "public"."vote_results_result_enum" NOT NULL;
                    COMMENT ON COLUMN "vote_results"."result" IS '의결 결과 (accepted|rejected)';
                END IF;
            END $$;
        `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                    WHERE table_name='dismissed_permission_logs' AND column_name='logType') 
                THEN
                    ALTER TABLE "dismissed_permission_logs" ADD "logType" "public"."dismissed_permission_logs_logtype_enum" NOT NULL;
                    COMMENT ON COLUMN "dismissed_permission_logs"."logType" IS '로그 타입 (announcement | wiki)';
                END IF;
            END $$;
        `);
        
        // categoryId nullable 처리는 새 마이그레이션에서 처리되므로 스킵
        ...
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "idx_wiki_file_system_type" ON "wiki_file_systems" ("type") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "idx_wiki_permission_log_action" ON "wiki_permission_logs" ("action") `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "idx_category_entity_type" ON "categories" ("entityType") `);
        ...
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS "idx_dismissed_permission_log_type_id" ON "dismissed_permission_logs" ("logType", "permissionLogId") `);
    }
