...
describe('[E2E] POST /api/admin/irs - 파일 업로드', () => {
  ...
  afterAll(async () => {
    ...
    if (createdIRId) {
      await testHelper
        .request()
        .delete(`/api/admin/irs/${createdIRId}`)
        .expect(200);
    }
  ...
  describe('파일 업로드 테스트', () => {
    it('파일과 함께 IR을 생성할 수 있어야 한다', async () => {
      // 테스트용 PDF 파일 생성
      const testFileContent = 'This is a test IR PDF for E2E testing.';
      const testFileName = 'test-ir.pdf';
      ...
      expect(ir.attachments.length).toBe(1);
    ...
    it('여러 파일을 동시에 업로드할 수 있어야 한다', async () => {
      ...
      // 파일 2개 업로드 확인
      expect(ir.attachments).toBeDefined();
      expect(ir.attachments).not.toBeNull();
      expect(ir.attachments!.length).toBe(2);

      // 각 파일 검증
      const fileNames = ir.attachments!.map(a => a.fileName);
      ...
      await testHelper
        .request()
        .delete(`/api/admin/irs/${ir.id}`)
        .expect(200);
    });

    it('기존 IR에 파일을 추가할 수 있어야 한다', async () => {
      // 먼저 파일 없이 IR 생성
