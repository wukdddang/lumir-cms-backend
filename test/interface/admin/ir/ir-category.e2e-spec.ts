...
describe('IR 카테고리 E2E 테스트', () => {
  ...
  describe('POST /api/admin/irs/categories (카테고리 생성)', () => {
    it('IR 카테고리를 생성해야 한다', async () => {
      // Given
      const createDto = {
        name: '재무제표',
  ...
  describe('PATCH /api/admin/irs/categories/:id (카테고리 수정)', () => {
    it('카테고리를 수정해야 한다', async () => {
      ...
      const updateResponse = await testSuite
        .request()
        .patch(`/api/admin/irs/categories/${categoryId}`)
        .send(updateDto)
        .expect(200);
    ...
    it('존재하지 않는 카테고리 수정 시 404 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .patch('/api/admin/irs/categories/00000000-0000-0000-0000-000000000001')
        .send(updateDto)
        .expect(404);
    });
  });

  describe('PATCH /api/admin/irs/categories/:id/order (카테고리 순서 변경)', () => {
    it('카테고리 순서를 변경해야 한다', async () => {
      ...
      const updateResponse = await testSuite
        .request()
        .patch(`/api/admin/irs/categories/${categoryId}/order`)
        .send(updateOrderDto)
        .expect(200);
    ...
    it('존재하지 않는 카테고리 순서 변경 시 404 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .patch('/api/admin/irs/categories/00000000-0000-0000-0000-000000000001/order')
        .send(updateOrderDto)
        .expect(404);
    });
  });

  describe('DELETE /api/admin/irs/categories/:id (카테고리 삭제)', () => {
    it('카테고리를 삭제해야 한다', async () => {
      ...
      const deleteResponse = await testSuite
        .request()
        .delete(`/api/admin/irs/categories/${categoryId}`)
        .expect(200);
    ...
    it('존재하지 않는 카테고리 삭제 시 404 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .delete('/api/admin/irs/categories/00000000-0000-0000-0000-000000000001')
        .expect(404);
    });
  ...
  describe('복합 시나리오', () => {
    ...
    it('여러 카테고리를 생성하고 목록을 조회해야 한다', async () => {
      ...
      expect(categoryNames).toContain('IR 자료');
    });
