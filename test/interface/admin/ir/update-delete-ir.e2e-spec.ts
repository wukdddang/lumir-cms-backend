...
describe('PUT/PATCH/DELETE /api/admin/irs (IR 수정/삭제)', () => {
  ...
  describe('PUT /api/admin/irs/:id (IR 수정)', () => {
    ...
    it('파일과 함께 IR을 수정해야 한다 (기존 파일 교체)', async () => {
      // Given - 파일이 있는 IR 생성
      const translationsData = [
        {
          languageId,
  ...
  describe('PATCH /api/admin/irs/:id/public (공개 상태 수정)', () => {
    it('IR을 비공개로 변경해야 한다', async () => {
      // Given - 공개 IR 생성
      const translationsData = [
        {
          languageId,
      ...
      const patchResponse = await testSuite
        .request()
        .patch(`/api/admin/irs/${irId}/public`)
        .send({ isPublic: false })
        .expect(200);
    ...
    it('IR을 공개로 변경해야 한다', async () => {
      // Given - 비공개 IR 생성 후 변경
      const translationsData = [
        {
          languageId,
          title: '비공개 테스트',
    ...
    it('존재하지 않는 ID로 공개 상태 수정 시 404 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .patch('/api/admin/irs/00000000-0000-0000-0000-000000000001/public')
        .send({ isPublic: false })
        .expect(404);
    });
  });

  describe('PUT /api/admin/irs/batch-order (순서 일괄 수정)', () => {
    it('여러 IR의 순서를 일괄 수정해야 한다', async () => {
      ...
      const batchOrderDto = {
        irs: [
          { id: ir3.body.id, order: 0 },
          { id: ir2.body.id, order: 1 },
          { id: ir1.body.id, order: 2 },
        ],
      };

      const updateResponse = await testSuite
        .request()
        .put('/api/admin/irs/batch-order')
        .send(batchOrderDto)
        .expect(200);
  ...
  describe('DELETE /api/admin/irs/:id (IR 삭제)', () => {
    it('IR을 삭제해야 한다 (Soft Delete)', async () => {
      // Given - IR 생성
      const translationsData = [
        {
          languageId,
    ...
    it('존재하지 않는 ID로 삭제 시 404 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .delete('/api/admin/irs/00000000-0000-0000-0000-000000000001')
        .expect(404);
    });

    it('이미 삭제된 IR을 다시 삭제 시 404 에러가 발생해야 한다', async () => {
      // Given - IR 생성 및 삭제
      const translationsData = [
        {
          languageId,
  ...
  describe('복합 시나리오', () => {
    it('IR을 생성, 수정, 공개 변경, 순서 변경, 삭제하는 전체 플로우를 테스트해야 한다', async () => {
      // 1. 생성
      const translationsData = [
        {
          languageId,
